...
// Lazy-load fluent-ffmpeg only when needed
function getFFmpeg() {
  if (!ffmpeg) {
...
function generatePromptVideo(inputPath, outputPath, prompt, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    // Build the ffmpeg command with FIXED filter syntax
    const ff = getFFmpeg();
    const cmd = ff(inputPath)
      .output(outputPath)
      .audioCodec("aac")
      ...
      .outputOptions([
        `-filter_complex`,
        `[0:a]showspectrum=s=1280x520:mode=combined:color=wave:scale=cbr:slide=scroll:win_func=hann[spec];` +
