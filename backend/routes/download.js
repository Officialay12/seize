...
function getStrategies(platform) {
  ...
  switch (platform) {
    ...
    case "pinterest":
      // Pinterest needs multiple strategies with different approaches
      return [
        // Strategy 1: With cookies, desktop UA
        {
          ...base,
        ...
        // Strategy 2: Without cookies, desktop UA
        {
          ...base,
          cookies: undefined,
          extractorArgs: "generic",
        ...
        // Strategy 3: With cookies, mobile UA
        {
          ...base,
          extractorArgs: "generic",
          addHeaders: {
...
// ============================================================
// DIRECT PINTEREST EXTRACTION - bypass yt-dlp entirely
// ============================================================
  ...
  for (const ua of USER_AGENTS) {
    try {
      ...
      const videoPatterns = [
        ...
        // Pinterest's JSON data
        /"videoList"\s*:\s*\[\s*\{[^}]*"url"\s*:\s*"([^"]+)"/i,
        /"story_pin_data"[^}]*"video"[^}]*"url"\s*:\s*"([^"]+)"/i,
      ];
...
async function extractTikTokDirect(url) {
  ...
  try {
    ...
    const videoMatch = embedHtml.match(/https:\/\/[^\s"]+\.mp4[^\s"]*/i);
    ...
    return {
      ...
      hasImage: false,
      media: {
        videos: [{ url: videoMatch[0], format: "mp4", quality: "HD" }],
        images: [],
        audio: [],
      },
      directExtract: true,
    ...
    console.log("[seize] TikTok direct extraction failed:", err.message);
    return null;
  }
}

// ============================================================
// DIRECT INSTAGRAM EXTRACTION
// ============================================================
async function extractInstagramDirect(url) {
  ...
  try {
    ...
    // Try to get video URL from thumbnail URL
    const thumbnailUrl = data.thumbnail_url;
    const videoUrl = thumbnailUrl.replace(/\.jpg$/, ".mp4");
    ...
    return {
      ...
      contentType: data.thumbnail_url ? "video" : "image",
      hasVideo: !!data.thumbnail_url,
      hasImage: true,
      media: {
        videos: data.thumbnail_url
          ? [{ url: videoUrl, format: "mp4", quality: "HD" }]
          : [],
        images: [{ url: thumbnailUrl, format: "jpg" }],
        audio: [],
      },
      directExtract: true,
    };
  } catch (err) {
    console.log("[seize] Instagram direct extraction failed:", err.message);
    return null;
  }
}
...
async function resolveWithStrategies(url, platform, isUsable) {
  // Try direct extraction first for certain platforms
  ...
  if (platform === "tiktok") {
    const directResult = await extractTikTokDirect(url);
    if (directResult) {
      return { info: directResult, strategyIndex: -1, directExtract: true };
    }
  }
...
function extractMediaUrls(info) {
  ...
  // If it's a direct extraction result, use it
  ...
  for (const node of nodes) {
    ...
    if (Array.isArray(node.formats)) {
      for (const format of node.formats) {
        ...
        const isTwitterGif =
          format.format_note && /(gif|animated)/i.test(format.format_note);

        if (
          isVideoFormat ||
          (isVideoExt && (isSnapchatVideo || isGifFormat || isTwitterGif))
        ) {
          media.videos.push({
            url: format.url,
            ...
            isGif: isGifFormat || isTwitterGif,
          });
          media.hasVideo = true;
          if (isGifFormat || isTwitterGif) media.isGif = true;
        }
...
function isUsableInfo(info) {
  ...
  // Direct extraction is always usable
  if (info.directExtract) {
